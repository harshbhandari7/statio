...
def get_status(db: Session = Depends(get_db)):
    ...
    for incident in incidents:
        timeline.append({
            ...
            "status": str(incident.status.value) if incident.status else "unknown",
        ...
        if incident.updates and len(incident.updates) > 0:
            ...
            timeline.append({
                ...
                "status": str(latest_update.status.value) if latest_update.status else "unknown",
    ...
    for maintenance in maintenances:
        timeline.append({
            ...
            "status": str(maintenance.status.value) if maintenance.status else "unknown",
    ...
    for service in services:
        if service.updated_at and service.updated_at >= recent_window:
            timeline.append({
                ...
                "status": str(service.status.value) if service.status else "unknown",
